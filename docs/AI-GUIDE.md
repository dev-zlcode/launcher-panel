...
**手势矩阵**（唯一口径，README 的「条目与手势」就是这张表，改行为要同时改两处）：
...
「有效清单」＝条目自己的候选 → 该类型清单（只有 `folder`/`file`/`url` 三张）→ 都没有才交给 macOS；**`app` 两级都不参与**，恒等于「系统默认＝启动它自己」，详见 §4。

**命令面板（⌘K）不套用这张表**：面板是"找东西"的地方，行上**单击＝定位**（`locate()`：关面板、必要时清掉侧栏筛选与页头搜索把卡片挪进视图、`card-<id>` 滚到中间并框 1.8s），**双击和右键都没有含义**（右键那份菜单只挂在卡片上，面板行没有 `onContextMenu`，也没有行内「编辑」）。执行动作走 Enter / ⌘Enter，用的是卡片同一层 `perform()` 分派：**片段→复制、命令→运行、其余→默认打开**，只是不等 250ms。别把面板的 Enter 直接接到 `/api/open` 上——命令条目会被 400（`命令条目要用「运行」`），片段条目是 no-op，表现成"按了没反应"。也别把 250ms 让位和双击菜单搬进面板。

| kind | `value` 填什么 | 要注意 |
| --- | --- | --- |
| `app` | `.app` **绝对路径**（`~` 会展开） | 名称留空取路径 basename |
| `folder` | 目录绝对路径 | |
| `file` | 文件绝对路径 | |
| `url` | `http`/`https` 链接 | 其他协议 400 `只允许 http/https` |
| `snippet` | 任意文本 | 复制走 `pbcopy`；`/api/open` 对它是 no-op，返回 `{ok:true,note:'snippet 走复制'}` |
| `command` | 可以**多行**的 shell | 后台 `zsh -i -c` 执行，输出进 `data/runs/<id>.log`；`/api/open` 会 400 `命令条目要用「运行」`，必须走 `/api/run` |
...
- `app` **完全不参与候选**（`canPickApps()` / `ownApps()`，`src/paths.ts`）：`open -a 候选 X.app` 等于把一个应用当文档交给另一个应用，没有意义。所以读侧一律当它没有候选（角标、失效徽标、双击菜单、有效清单全部为空，单击＝`open 自身`），写侧编辑器不给「候选打开应用」、右键不给「打开方式…」、保存一律写 `openWith: []`。老数据里残留的候选不会再生效，只在设置页「已失效引用」里还能被列出来清理。
